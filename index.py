...
@app.route("/payroll/<a>")
def payroll(a):
    allpayrolls = Employee.select().join(Payroll).where(Payroll.id == int(a))

    return render_template("payroll.html", myPayrolls=allpayrolls, id=a)



# @app.route("/deletepayroll")
# def deletepayroll(b):
#     payrolinstance = payrolls.select().where(employee.id == int(b)).get()
#     payrolinstance.delete_instance()
#
#     return render_template("payroll.html", myPayrolls=payrolinstance, id=b)
